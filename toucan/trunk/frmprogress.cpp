...
////@begin frmProgress content construction
    // Generated by DialogBlocks, 30/07/2007 13:43:31 (Personal Edition)
...
//Working here
	...
	{
		  command = wxT("App\\toucan\\7za.exe a -t") + strFormat + wxT(" -mx") + strRatio + wxT(" \"") + strSecond + wxT("\" \"") + strFirst + wxT("\\*\"") + wxT(" -x@\"") + wxPathOnly(wxStandardPaths::Get().GetExecutablePath()) + wxFILE_SEP_PATH + wxT("App") +   wxFILE_SEP_PATH + wxT("toucan") + wxFILE_SEP_PATH + wxT("temp-exclusions.txt") + wxT("\"");    
	}
	else if(strType == _("Update"))
	{
        command = wxT("App\\toucan\\7za.exe  u -t") + strFormat + wxT(" \"") + strSecond + wxT("\" \"") + strFirst + wxT("\\*\"")  + wxT(" -x@\"") + wxPathOnly(wxStandardPaths::Get().GetExecutablePath()) + wxFILE_SEP_PATH + wxT("App") +   wxFILE_SEP_PATH + wxT("toucan") + wxFILE_SEP_PATH + wxT("temp-exclusions.txt") + wxT("\"");;
	}
	else if(strType == _("Restore"))
	{
        command = wxT("App\\toucan\\7za.exe  x  \"") + strFirst + wxT("\" -o\"") + strSecond + wxT("\" * -r")  + wxT(" -x@\"") + wxPathOnly(wxStandardPaths::Get().GetExecutablePath()) + wxFILE_SEP_PATH + wxT("App") +   wxFILE_SEP_PATH + wxT("toucan") + wxFILE_SEP_PATH + wxT("temp-exclusions.txt") + wxT("\"");;
	}
    else if(strType == _("Incremental")){
        if(wxFileExists(strSecond + wxFILE_SEP_PATH + wxT("BaseFile.") + strFormat))
        {
            wxDateTime now = wxDateTime::Now();
            command = wxT("App\\toucan\\7za.exe u \"") + strSecond + wxFILE_SEP_PATH + wxT("BaseFile.") + strFormat + wxT("\" -u- -up0q3x2z0!\"") + strSecond + wxFILE_SEP_PATH + now.FormatISODate() + wxT(".") + strFormat + wxT("\" \"") + strFirst + wxT("\\*\"") + wxT(" -x@\"") + wxPathOnly(wxStandardPaths::Get().GetExecutablePath()) + wxFILE_SEP_PATH + wxT("App") +   wxFILE_SEP_PATH + wxT("toucan") + wxFILE_SEP_PATH + wxT("temp-exclusions.txt") + wxT("\"");
        //wxMessageBox(command);
        }
        else
        {
        		  command = wxT("App\\toucan\\7za.exe a -t") + strFormat + wxT(" -mx") + strRatio + wxT(" \"") + strSecond + wxFILE_SEP_PATH + wxT("BaseFile.") + strFormat + wxT("\" \"") + strFirst + wxT("\\*\"")  + wxT(" -x@\"") + wxPathOnly(wxStandardPaths::Get().GetExecutablePath()) + wxFILE_SEP_PATH + wxT("App") +   wxFILE_SEP_PATH + wxT("toucan") + wxFILE_SEP_PATH + wxT("temp-exclusions.txt") + wxT("\"");    
               // wxMessageBox(command);       
        }
