...
wxString CreateCommand(wxString strFirst, wxString strSecond, wxString strType, wxString strFormat, wxString strRatio, wxString strPass)
{
    ...
    if(strType == _("Complete")){
        command = wxT("App\\toucan\\7za.exe a -t") + strFormat + strPass + wxT(" -mx") + strRatio + wxT(" \"") + strSecond + wxT("\" \"") + strFirst + wxT("\\*\"") + wxT(" -x@\"") + wxPathOnly(wxStandardPaths::Get().GetExecutablePath()) + wxFILE_SEP_PATH + wxT("App") +   wxFILE_SEP_PATH + wxT("toucan") + wxFILE_SEP_PATH + wxT("temp-exclusions.txt") + wxT("\"") ;    
        //wxMessageBox(command);
    }
	else if(strType == _("Update")){
        command = wxT("App\\toucan\\7za.exe  u -t") + strFormat + wxT(" \"") + strSecond + wxT("\" \"") + strFirst + wxT("\\*\"")  + wxT(" -x@\"") + wxPathOnly(wxStandardPaths::Get().GetExecutablePath()) + wxFILE_SEP_PATH + wxT("App") +   wxFILE_SEP_PATH + wxT("toucan") + wxFILE_SEP_PATH + wxT("temp-exclusions.txt") + wxT("\"")  + strPass;
	}
	else if(strType == _("Restore")){
        command = wxT("App\\toucan\\7za.exe  x  \"") + strFirst + wxT("\" -o\"") + strSecond + wxT("\" * -r")  + wxT(" -x@\"") + wxPathOnly(wxStandardPaths::Get().GetExecutablePath()) + wxFILE_SEP_PATH + wxT("App") +   wxFILE_SEP_PATH + wxT("toucan") + wxFILE_SEP_PATH + wxT("temp-exclusions.txt") + wxT("\"") + strPass;
	}
	//With the incremental type the first use creates a file called base file. On subsequent runs a file is created with a filename based on both the date and time.    
        ...
        if(wxFileExists(strSecond + wxFILE_SEP_PATH + wxT("BaseFile.") + strFormat)){
            ...
            command = wxT("App\\toucan\\7za.exe u \"") + strSecond + wxFILE_SEP_PATH + wxT("BaseFile.") + strFormat + wxT("\" -u- -up0q3x2z0!\"") + strSecond + wxFILE_SEP_PATH + now.FormatISODate() + wxT("-") +  now.Format(wxT("%H")) + wxT("-") +  now.Format(wxT("%M")) + wxT(".") + strFormat + wxT("\" \"") + strFirst + wxT("\\*\"") + wxT(" -x@\"") + wxPathOnly(wxStandardPaths::Get().GetExecutablePath()) + wxFILE_SEP_PATH + wxT("App") +   wxFILE_SEP_PATH + wxT("toucan") + wxFILE_SEP_PATH + wxT("temp-exclusions.txt") + wxT("\"") + strPass;
        }
        else{
        		  command = wxT("App\\toucan\\7za.exe a -t") + strFormat + wxT(" -mx") + strRatio + wxT(" \"") + strSecond + wxFILE_SEP_PATH + wxT("BaseFile.") + strFormat + wxT("\" \"") + strFirst + wxT("\\*\"")  + wxT(" -x@\"") + wxPathOnly(wxStandardPaths::Get().GetExecutablePath()) + wxFILE_SEP_PATH + wxT("App") +   wxFILE_SEP_PATH + wxT("toucan") + wxFILE_SEP_PATH + wxT("temp-exclusions.txt") + wxT("\"") + strPass;       
        }
    }
    return command;
}
