...
m_Backup_Text_2->SetMinSize(wxSize(100,-1));
	...
	m_Backup_Pass = new wxTextCtrl( itemPanel33, ID_BACKUPPASS, _T(""), wxDefaultPosition, wxSize(-1, -1), wxTE_PASSWORD );
	itemStaticBoxSizer100->Add(m_Backup_Pass, 0, wxALIGN_CENTER_VERTICAL|wxALL, 5);
	m_Backup_Pass2 = new wxTextCtrl( itemPanel33, ID_BACKUPPASS2, _T(""), wxDefaultPosition, wxSize(-1, -1), wxTE_PASSWORD );
	...
	m_RoutineSecure->SetToolTip(_("You can choose either AES or Blowfish encryption. AES is recommended as it is faster and more secure. \nFiles encrypted using Toucan 1.1 or previous used Blowfish and therefore Blowfish must be used to decrypt them. AES also has the benefit of appending an entension to encrypted files so you can be sure they are encrypted."));
...
if(m_Backup_Pass->GetValue() == m_Backup_Pass2->GetValue())
{
	if(m_Backup_First->GetValue() != wxEmptyString && m_Backup_Second->GetValue() != wxEmptyString){
		...
		if(strFirst != wxEmptyString && strSecond  != wxEmptyString){
			...
			//Run the backup
			Backup(strFirst, strSecond, m_BackupType->GetStringSelection(), m_BackupFormat->GetStringSelection(), m_BackupRatio->GetStringSelection(), true, m_Backup_Pass->GetValue());
		}
	}
	else{
		wxMessageBox(_("You are missing a required field please try again"), _("Error"), wxICON_ERROR);
...
wxFileConfig *config = new wxFileConfig( wxT(""), wxT(""), wxPathOnly(wxStandardPaths::Get().GetExecutablePath()) + wxT("\\Data\\Jobs.ini") );
        ...
        config->Write(strName+ wxT("/Password"), m_Backup_Pass->GetValue());
...
{	    
    ...
    if(window->ShowModal() == wxID_OK)
    {
        ...
        m_Backup_Pass->SetValue(config->Read(strName+ wxT("/Password")));
